...
namespace audio_transport
{
  class RosGstCapture
  {
	...
	_convert2 = gst_element_factory_make("audioconvert", "convert2");
	_resample = gst_element_factory_make("audioresample", "resample");
	_vader = gst_element_factory_make("vader", "vad");
	g_object_set(G_OBJECT(_vader), "auto-threshold", TRUE, NULL);
        ...
        gst_bin_add_many( GST_BIN(_pipeline), _source, _convert, _resample, _vader, _convert2, _encode, _sink, NULL);
        gst_element_link_many(_source, _convert, _resample, _vader, _convert2, _encode, _sink, NULL);
        if (!_source || !_convert || !_encode || !_sink) {
		ROS_ERROR("Creating an element failed: _source:%d _convert:%d \
			_resample:%d _vader:%d _convert2:%d _encode:%d _sink:%d"
			, !!_source
			, !!_convert
			, !!_resample
			, !!_vader
    ...
    private:
      ...
      GstElement *_pipeline, *_source, *_sink, *_convert, *_convert2, *_encode, *_vader, *_resample;
      GMainLoop *_loop;
      int _bitrate;
  };
}
